# Uncomment this and the zprof line at the end of the file for profiling
# zmodload zsh/zprof
...
# nvm (generated by node installer)
# NOTE: I use the nvm omz plugin to lazy load nvm, which otherwise delays shell
# startup by nearly 2 seconds. See plugins above. Since that also waits to load
# completions, we can eagerly run just that part here
...
# Always use _files completion for delta, which otherwise doesn't autocomplete
# a second file name. Since the only way I use delta directly is to compare two
# files, this is the only completion I want
...
# UPDATE: Most of this seems to be obsolete. fzf now auto-generates its shell
# integration config. We do need to tell it where fzf lives, though
...
# Uncomment this and the zprof line at the top of the file for profiling
# zprof
